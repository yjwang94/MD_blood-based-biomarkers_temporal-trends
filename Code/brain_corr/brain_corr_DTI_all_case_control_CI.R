...
#处理缺失值：delete
#DTI_mean = na.omit(DTI_mean)
#标准化 这一步很慢
#mean_DTI_area = data.frame(matrix(nrow = 1,ncol = length(colnames(DTI_mean))))
#for (i in c(2:length(colnames(DTI_mean)))) {
#  mean_DTI_area[i] =  mean(na.omit(DTI_mean[,i]))
#}
#sd_DTI_area = data.frame(matrix(nrow = 1,ncol = length(colnames(DTI_mean))))
#for (i in c(2:length(colnames(DTI_mean)))) {
#  sd_DTI_area[i] =  sd(na.omit(DTI_mean[,i]))
#}
#for (i in c(2:length(colnames(DTI_mean)))) {
#  for (j in c(1:length(rownames(DTI_mean)))) {
#    DTI_mean[j,i] = (DTI_mean[j,i]-mean_DTI_area[,i])/sd_DTI_area[,i]
#  }
#}
DTI_mean1 = scale(DTI_mean[2:length(colnames(DTI_mean))])
DTI_mean = cbind(DTI_mean[1],DTI_mean1)
#write.csv(DTI_mean,"/Users/yujia/Desktop/a/project/Evolution/MD/MD_review/data/Neuroimaging/DTI_mean.csv",row.names = FALSE)

#############
#DTI_mean = read.csv("/Users/yujia/Desktop/a/project/Evolution/MD/MD_review/data/Neuroimaging/DTI_mean.csv")
...
MD = c("BIP")
betas_all = data.frame(matrix(nrow = 45))
pvalues_all = data.frame(matrix(nrow = 45))
low_CI_all = data.frame(matrix(nrow = 45))
up_CI_all = data.frame(matrix(nrow = 45))
for (MD in MD_list) {
  fpath = paste("/Users/yujia/Desktop/a/project/Evolution/MD/MD_review/data/CaseControl/",MD,"/S4_data_pheno_sig.csv",sep = "")
  data_pheno_sig = read.csv(fpath)
  colnames(data_pheno_sig)[2] = "duration"
  ...
  significant_variable_sorted <- arrange(significant_variable, cluster1)
  ...
  #拟合线性模型
  betas_data = data.frame(matrix(nrow = 45))
  pvalues_data = data.frame(matrix(nrow = 45))
  low_CI_data = data.frame(matrix(nrow = 45))
  up_CI_data = data.frame(matrix(nrow = 45))
  for (i in (48:(47+length(significant_variable_lst)))){#blood biomarkers
    betas = data.frame()
    pvalues = data.frame()
    low_CIs = data.frame()
    up_CIs = data.frame()
    for (j in ((48+length(significant_variable_lst)):(length(significant_variable_lst)+92))) {#脑区
      complete = MD_pheno_sig_volume_DKT_mean_cov[complete.cases(MD_pheno_sig_volume_DKT_mean_cov[,i]),]
      lm1 = lm(complete[,j] ~ complete[,'group']*complete[,i]+ Gender + TD_Index + Age + SMK_Status1 + SMK_Status2  +
      ...
      beta = lm1$coefficients['complete[, "group"]:complete[, i]']
      sumres <- summary(lm1)
      pvalue = sumres$coefficients['complete[, "group"]:complete[, i]','Pr(>|t|)']
      low_CI = confint(lm1, level = 0.95)['complete[, "group"]:complete[, i]',1]
      up_CI = confint(lm1, level = 0.95)['complete[, "group"]:complete[, i]',2]
  ...
  rownames(betas_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov)[(48+length(significant_variable_lst)):(length(significant_variable_lst)+92)]
  colnames(pvalues_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov)[48:(47+length(significant_variable_lst))]
  rownames(pvalues_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov)[(48+length(significant_variable_lst)):(length(significant_variable_lst)+92)]
  ...
  rownames(low_CI_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov)[(48+length(significant_variable_lst)):(length(significant_variable_lst)+92)]
  colnames(up_CI_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov)[48:(47+length(significant_variable_lst))]
  rownames(up_CI_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov)[(48+length(significant_variable_lst)):(length(significant_variable_lst)+92)]
  rownames(low_CI_data) = capitalize(rownames(low_CI_data))
  rownames(up_CI_data) = capitalize(rownames(up_CI_data))
...
colnames(betas_all)[1] = "WBC"
colnames(pvalues_all)[1] = "WBC"
colnames(low_CI_all)[1] = "WBC"
colnames(up_CI_all)[1] = "WBC"
...
ggsave(paste("DTI_all_sig_new.pdf",sep = ""),width = 10,height = 12)
