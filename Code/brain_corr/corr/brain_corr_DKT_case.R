...
volume_DKT_mean1 = scale(volume_DKT_mean[2:length(colnames(volume_DKT_mean))])
volume_DKT_mean = cbind(volume_DKT_mean[1],volume_DKT_mean1)
...
data_cov_pheno = MD_pheno_sig_volume_DKT_mean_cov
data_cov_pheno_elimate = MD_pheno_sig_volume_DKT_mean_cov
for (i in ((48+length(significant_variable_sorted_lst)):(length(data_cov_pheno)-1))){
...
MD_pheno_sig_volume_DKT_mean_cov_elimate = data_cov_pheno_elimate
#画热图
library(reshape2)
library(dplyr)
library(ggplot2)
...
for (i in c(48:(47+length(significant_variable_lst)))) {
  corrs = data.frame()
  pvalues = data.frame()
  for (j in c((48+length(significant_variable_lst)):(length(significant_variable_lst)+86))) {
...
colnames(corrs_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov_elimate)[48:(47+length(significant_variable_lst))]
rownames(corrs_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov_elimate)[(48+length(significant_variable_lst)):(length(significant_variable_lst)+86)]
colnames(pvalues_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov_elimate)[48:(47+length(significant_variable_lst))]
rownames(pvalues_data) = colnames(MD_pheno_sig_volume_DKT_mean_cov_elimate)[(48+length(significant_variable_lst)):(length(significant_variable_lst)+86)]
...
ggsave(paste("DKT_",MD,"_case.pdf",sep = ""),width = 15,height = 10)
