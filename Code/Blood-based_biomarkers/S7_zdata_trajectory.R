...
clustered_data = read.csv(paste("/Users/yujia/Desktop/a/project/Evolution/MD/MD_review/data/CaseControl/S6_cluster_fcm/S62_cluster_data_",MD,".csv",sep = ""))
...
setwd(paste("/Users/yujia/Desktop/a/project/Evolution/MD/MD_review/data/CaseControl/",MD,"/trend_cluster_fcm/",sep = ''))
...
for (i in c(1:8)) {
  ...
  p_cluster <- ggplot() +
    geom_line(stat="smooth", data=zdata_plot_melt, aes(x=duration, y=value, group=variable),method = 'loess', span = 2,se = FALSE,linewidth = 1,alpha = 0.4,color = color[i]) +
    labs(x="Years to Diagnosis", y="Z-Score") +
    scale_x_continuous(limits=c(-10,10),breaks = seq(-10, 10, by = 2)) +
    geom_smooth(data=zdata_plot_melt, aes(x=duration, y=value),method = 'loess', span = 2, se = FALSE,linewidth = 6, show.legend = TRUE,color = color[i])+
